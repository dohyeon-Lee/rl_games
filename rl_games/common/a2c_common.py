...
class A2CBase(BaseAlgorithm):
    ...
    def __init__(self, base_name, params):
        ...
        pbt_str = ''
        self.population_based_training = config.get('population_based_training', False) # TODO default false
        ...
        full_experiment_name = config.get('full_experiment_name', None) # TODO default None
        ...
        self.config = config
        self.algo_observer = config['features']['observer'] # TODO MultiObserver(observers), observers = [RLGPUAlgoObserver()], define in torch_runner 
        ...
        self.use_diagnostics = config.get('use_diagnostics', False) # TODO defaiult false
        ...
        self.network_path = config.get('network_path', "./nn/") # TODO if not 'network_path' in config, return "./nn/"
        self.log_path = config.get('log_path', "runs/")
        self.env_config = config.get('env_config', {})
        self.num_actors = config['num_actors'] # TODO numEnvs
        ...
        self.use_action_masks = config.get('use_action_masks', False) # TODO False default
        self.is_train = config.get('is_train', True) # TODO True default

        self.central_value_config = self.config.get('central_value_config', None) # TODO default false
        self.has_central_value = self.central_value_config is not None  # TODO None default
        self.truncate_grads = self.config.get('truncate_grads', False) # TODO default True
        ...
        self.self_play_config = self.config.get('self_play_config', None) # TODO default False
        self.has_self_play_config = self.self_play_config is not None # TODO default False

        self.self_play = config.get('self_play', False) # TODO false
        self.save_freq = config.get('save_frequency', 0)  #TODO 50
        self.save_best_after = config.get('save_best_after', 100) # TODO 200
        self.print_stats = config.get('print_stats', True) # TODO True
        self.rnn_states = None
        self.name = base_name # TODO 'run'

        self.ppo = config.get('ppo', True) # TODO True
        self.max_epochs = self.config.get('max_epochs', -1) # TODO maybe TODO default 1000
        self.max_frames = self.config.get('max_frames', -1) # TODO default -1

        self.is_adaptive_lr = config['lr_schedule'] == 'adaptive' # TODO default True
        self.linear_lr = config['lr_schedule'] == 'linear' # TODO default false
        self.schedule_type = config.get('schedule_type', 'legacy') # TODO default legacy

        # Setting learning rate scheduler
        if self.is_adaptive_lr:
            self.kl_threshold = config['kl_threshold'] # TODO default 0.008
        ...
        self.e_clip = config['e_clip'] # TODO 0.2
        self.clip_value = config['clip_value'] # TODO true
        self.network = config['network'] # TODO cal when do load network , ModelA2CContinuous
        self.rewards_shaper = config['reward_shaper'] # TODO scale_value: 1.0
        self.num_agents = self.env_info.get('agents', 1) # TODO maybe TODO default 1
        self.horizon_length = config['horizon_length'] # TODO 24
        self.seq_len = self.config.get('seq_length', 4) # TODO default 4
        self.bptt_len = self.config.get('bptt_length', self.seq_len) # not used right now. Didn't show that it is usefull
        self.zero_rnn_on_done = self.config.get('zero_rnn_on_done', True) # TODO default True
        self.normalize_advantage = config['normalize_advantage'] # TODO default True
        self.normalize_rms_advantage = config.get('normalize_rms_advantage', False) # TODO false
        self.normalize_input = self.config['normalize_input'] # TODO true
        self.normalize_value = self.config.get('normalize_value', False) # TODO True
        self.truncate_grads = self.config.get('truncate_grads', False) # TODO True
        ...
        self.critic_coef = config['critic_coef'] # TODO default 2
        self.grad_norm = config['grad_norm'] # TODO default 1.
        self.gamma = self.config['gamma'] # TODO 0.99
        self.tau = self.config['tau'] # TODO 0.95

        self.games_to_track = self.config.get('games_to_track', 100) # TODO default 100
        ...
        assert(('minibatch_size_per_env' in self.config) or ('minibatch_size' in self.config)) # TODO minibatch_size = 32768
        self.minibatch_size_per_env = self.config.get('minibatch_size_per_env', 0) # TODO 0
        self.minibatch_size = self.config.get('minibatch_size', self.num_actors * self.minibatch_size_per_env) # TODO 32768
        self.mini_epochs_num = self.config['mini_epochs'] # TODO default 5
        self.num_minibatches = self.batch_size // self.minibatch_size # TODO 24*4096 // 32768
        assert(self.batch_size % self.minibatch_size == 0) 

        self.mixed_precision = self.config.get('mixed_precision', False) # TODO True
        self.scaler = torch.cuda.amp.GradScaler(enabled=self.mixed_precision) # TODO for accelerate learning?

        self.last_lr = self.config['learning_rate'] # TODO 3.e-4
        self.frame = 0
        self.update_time = 0
        ...
        self.train_dir = config.get('train_dir', 'runs') # TODO default 'runs'
        ...
        self.entropy_coef = self.config['entropy_coef'] # TODO default 0.0

        if self.global_rank == 0: # TODO True
        ...
        self.value_bootstrap = self.config.get('value_bootstrap')  # TODO True
        self.use_smooth_clamp = self.config.get('use_smooth_clamp', False) # TODO false

        if self.use_smooth_clamp:
            self.actor_loss_func = common_losses.smoothed_actor_loss
        else:
            self.actor_loss_func = common_losses.actor_loss # TODO PPO loss calculate fuct

        if self.normalize_advantage and self.normalize_rms_advantage: # TODO false because normalize_rms_advantage is false
        ...
        #self_play
        if self.has_self_play_config: # TODO default false
        ...
        self.soft_aug = config['features'].get('soft_augmentation', None) # TODO maybe TODO None
    ...
    def load_networks(self, params):
        ...
        self.config['network'] = builder.load(params) # TODO ModelA2CContinuousLogStd
        has_central_value_net = self.config.get('central_value_config') is not  None # TODO default false
    ...
    def set_eval(self):
        self.model.eval() 
        if self.normalize_rms_advantage: # TODO false
    ...
    def init_tensors(self):
        ...
        algo_info = {
            ...
            'horizon_length' : self.horizon_length, # TODO 24
            'has_central_value' : self.has_central_value, # TODO none
            'use_action_masks' : self.use_action_masks # TODO False
        }
        self.experience_buffer = ExperienceBuffer(self.env_info, algo_info, self.ppo_device)

        val_shape = (self.horizon_length, batch_size, self.value_size) # TODO value_size : 1 (default)
        ...
        if self.is_rnn: # TODO default false
            self.rnn_states = self.model.get_default_rnn_state()
            self.rnn_states = [s.to(self.ppo_device) for s in self.rnn_states]

            total_agents = self.num_agents * self.num_actors
            num_seqs = self.horizon_length // self.seq_len # TODO 24/4
    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length): # TODO default 24
            if self.use_action_masks: # TODO default false
            ...
            if self.has_central_value: # TODO false
            ...
            if self.value_bootstrap and 'time_outs' in infos: # TODO self.value_bootstrap is true
            ...
            #TODO if done env, reset reward and length
...
class DiscreteA2CBase(A2CBase):
    ...
    def train_epoch(self):
        ...
        for mini_ep in range(0, self.mini_epochs_num): # TODO default mini_epochs_num = 5
            ep_kls = []
...
class ContinuousA2CBase(A2CBase):
    ...
    def __init__(self, base_name, params):
        ...
        self.bounds_loss_coef = self.config.get('bounds_loss_coef', None) # TODO 0.001

        self.clip_actions = self.config.get('clip_actions', True) # TODO True
    ...
    def train_epoch(self):
        super().train_epoch() # send the information in the direction algo->environment
    ...
    def train(self):
        ...
        rep_count = 0
        self.obs = self.env_reset()
        self.curr_frames = self.batch_size_envs  # 24 * 4096 (horizon_length * env_num)

        if self.multi_gpu: # TODO default false
            print("====================broadcasting parameters")
            model_params = [self.model.state_dict()]
            dist.broadcast_object_list(model_params, 0)
            self.model.load_state_dict(model_params[0])
